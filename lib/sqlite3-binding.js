...
// 优先查找正确路径（子目录），回退到 binding 直接目录
const primaryPath = path.join(__dirname, '..', modulePath, `${binary.module_name}.node`);
const fallbackPath = path.join(__dirname, '..', 'binding', `${binary.module_name}.node`);
const bindingPath = fs.existsSync(primaryPath) ? primaryPath : fallbackPath;
